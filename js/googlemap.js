...
const INITIAL_LONGITUDE = -32;
const INITIAL_LATITUDE = 2.102628;
var initialZ = 10;
...
const BASE_URL = 'http://www.ie511.org/';
...
function initializeMap(mapId, coords) {
    ...
    if (!coords.hasOwnProperty("longitude")){
    ...
    var mode = location.search.substring(1).split("&")[0].split("=")[1];
    switch (mode){
        case "sr91":{
            init_sr91();
            break;
...
function addLaneClosure() {
    ...
    var xmlUrl = "http://www.ie511.org/data/lcs.kml?" + randomNum.toString();
    downloadUrl(xmlUrl, function(doc) {
        var xmlDoc = xmlParse(doc);
        var batch = [];
    ...
    xmlUrl = "http://www.ie511.org/data/lcs_additional.kml?" + randomNum.toString();
    downloadUrl(xmlUrl, function(doc) {
        var xmlDoc = xmlParse(doc);
        var batch = [];
